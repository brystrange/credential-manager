 ...
 */
export function validatePlatformInput(data: unknown): PlatformInput {
    ...
    if (name.length > 100) throw new Error("Platform name must be 100 characters or fewer.");

    const domain = typeof d.domain === "string" ? d.domain.trim() : "";
    if (domain.length > 255) throw new Error("Domain must be 255 characters or fewer.");

    const color = typeof d.color === "string" ? d.color.trim() : "#6366f1";
    if (!/^#[0-9a-fA-F]{6}$/.test(color)) throw new Error("Invalid colour hex.");
    ...
    if (logoUrl.length > 2048) throw new Error("Logo URL must be 2048 characters or fewer.");

    const link = typeof d.link === "string" ? d.link.trim() : "";
    if (link.length > 2048) throw new Error("Link must be 2048 characters or fewer.");
